...
/// 静的メンバ変数の実体
/// </summary>
const float WireObject::radius = 5.0f;				// 底面の半径
const float WireObject::prizmHeight = 8.0f;			// 柱の高さ
...
void WireObject::SetEye(XMFLOAT3 eye)
{
	WireObject::eye = eye;
...
void WireObject::SetTarget(XMFLOAT3 target)
{
	WireObject::target = target;

	UpdateViewMatrix();
}

void WireObject::CameraMoveVector(XMFLOAT3 move)
{
...
void WireObject::InitializeDescriptorHeap()
{
...
void WireObject::InitializeCamera(int window_width, int window_height)
{
	// ビュー行列の生成
...
void WireObject::LoadTexture(const std::string& directoryPath, const std::string& filename)
{
	//HRESULT result = S_FALSE;
...
void WireObject::UpdateViewMatrix()
{
	// ビュー行列の更新
	matView = XMMatrixLookAtLH(XMLoadFloat3(&eye), XMLoadFloat3(&target), XMLoadFloat3(&up));
}

bool WireObject::Initialize()
{
	// nullptrチェック
	assert(device);
...
void WireObject::LoadMaterial(const std::string& directoryPath, const std::string& filename)
{
	////ファイルストリーム
	//std::ifstream file;
	////マテリアルファイルを開く
