...
//静的メンバ変数の実体
const float PostEffect::clearColor[4] = { 0.25f,0.6f,0.1f,0.0f };//緑っぽい色
		...
		for (int i = 0; i < 2; i++) {
			...
			(
				...
				CD3DX12_CPU_DESCRIPTOR_HANDLE(
					...
					spriteCommon_->GetDirectXCommon()->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV)
				)
			);
		}
		//RTV用デスクリプタヒープ設定
...
#pragma region シェーダーコードのエラー
	...
	//rootParams[1].DescriptorTable.pDescriptorRanges = &descRangeSRV0; //デスクリプタレンジ
